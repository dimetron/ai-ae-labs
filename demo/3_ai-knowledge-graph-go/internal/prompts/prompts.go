// Package prompts provides prompt templates for LLM-based knowledge graph operations.
package prompts

import "fmt"

// MainSystem returns the system prompt for triple extraction.
func MainSystem() string {
	return `You are an advanced AI system specialized in knowledge extraction and knowledge graph generation.
...
// MainUser returns the user prompt for triple extraction.
func MainUser(text string) string {
	return fmt.Sprintf(`Your task: Read the text below (delimited by triple backticks) and identify all Subject-Predicate-Object (S-P-O) relationships in each sentence. Then produce a single JSON array of objects, each representing one triple.
...
// EntityResolutionSystem returns the system prompt for entity resolution.
func EntityResolutionSystem() string {
	return `You are an expert in entity resolution and knowledge representation.
Your task is to standardize entity names from a knowledge graph to ensure consistency.`
}

// EntityResolutionUser returns the user prompt for entity resolution.
func EntityResolutionUser(entityList string) string {
	return fmt.Sprintf(`Below is a list of entity names extracted from a knowledge graph.
...
// RelationshipInferenceSystem returns the system prompt for inter-community inference.
func RelationshipInferenceSystem() string {
	return `You are an expert in knowledge representation and inference.
Your task is to infer plausible relationships between disconnected entities in a knowledge graph.`
}

// RelationshipInferenceUser returns the user prompt for inter-community inference.
func RelationshipInferenceUser(entities1, entities2, triplesText string) string {
	return fmt.Sprintf(`I have a knowledge graph with two disconnected communities of entities.

Community 1 entities: %s
Community 2 entities: %s
...
IMPORTANT: Make sure the subject and object are different entities - avoid self-references.`, entities1, entities2, triplesText)
}

// WithinCommunitySystem returns the system prompt for intra-community inference.
func WithinCommunitySystem() string {
	return `You are an expert in knowledge representation and inference.
...
// WithinCommunityUser returns the user prompt for intra-community inference.
func WithinCommunityUser(pairsText, triplesText string) string {
	return fmt.Sprintf(`I have a knowledge graph with several entities that appear to be semantically related but are not directly connected.
...
Only include highly plausible relationships with clear predicates.
IMPORTANT: The inferred relationships (predicates) MUST be no more than 3 words maximum. Preferably 1-2 words. Never more than 3.
IMPORTANT: Make sure that the subject and object are different entities - avoid self-references.`, pairsText, triplesText)
}
